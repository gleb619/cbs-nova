...
/**
 * Factory methods for creating test transaction blocks. These methods return suspend lambdas that
 * can be called from Java tests.
 */
object TestTransactionBlocks {
  /** Creates a block that executes a single transaction step. */
  @JvmStatic
  fun singleStep(tx: TransactionDefinition): suspend TransactionsScope.() -> Unit = { step(tx) }
  ...
  @JvmStatic fun noOpBlock(): suspend TransactionsScope.() -> Unit = {}
}
