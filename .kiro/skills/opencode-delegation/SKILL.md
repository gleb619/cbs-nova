...
description: >
  Use when delegating implementation work to OpenCode as an executor agent.
  Kiro plans and writes a task spec, then runs OpenCode via bash, then verifies
  the result. Triggers when user asks to "use opencode", "delegate to opencode",
  "let opencode implement this", or when a task is well-scoped and can be handed
  off for autonomous execution.
---
...
Kiro acts as the **thinking center**: plans, writes a task spec, runs OpenCode
via `executeBash`, then verifies the result.
...
|------------|--------------|----------------------------------------------------------|
| `kiro`     | Orchestrator | Plan, write task spec, run OpenCode, verify result       |
...
[2. Kiro: Write docs/tasks/{task-name}.md]
      │
      ▼
[3. Kiro: Run OpenCode via executeBash]
...
[4. OpenCode: Execute task, write result file]
      │
      ▼
[5. Kiro: Read result file + re-run verification commands]
      │
      ├── PASS ──► Done ✅
      │
      └── FAIL ──► [6. Kiro: Amend task file, re-run OpenCode (retry #1)]
                         │
                         ▼
                   [7. Kiro: Verify again]
                         │
...
- Use `executeBash` — NOT `controlBashProcess`, NOT `invokeSubAgent`, NOT any MCP tool
- Set `timeout` to at least `600000` (10 minutes) — OpenCode takes time
- The command must be a single bash string (no `&&` splitting across calls)
- If opencode binary is not at `~/.opencode/bin/opencode`, try `which opencode` first

To find the opencode binary if the default path fails:
```bash
...
## Step 5 — Verification by Kiro

After OpenCode finishes, Kiro must:

1. **Read the result file** at `docs/results/{task-name}.result.md`
2. **Check Self-Assessment** — note if OpenCode flagged issues
3. **Re-run verification commands independently** via `executeBash`
...
- Before retrying, Kiro **must amend the task file** — append a `## Retry Notes` section:
...
2. Report to user: what was tried, exact failure, suggestion 
...
## Quick Reference Checklist (Kiro internal)
