...
| T46 | Code-based helper import via `#import` preprocessor                                      | `in-progress` | dsl     | T42, T43, T44  | T47      |
| T47 | Redesign `TestHelper` as real executable implementation with optional `execute` override | `todo`        | dsl     | T42            | T46      |
...
| 10 cont. | `parked` | T41     | E2E tests with `TestWorkflowEnvironment`                             |
| 11       | `todo`   | T42–T47 | Parameters/context for all DSL types + code import + real TestHelper |
