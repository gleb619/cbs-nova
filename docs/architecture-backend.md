...
A declarative piece manifest (`docs/vhs-manifest.md`) is being introduced as the single contract that describes each guarded piece, its target (an API route, a UI button, or a DSL object), ordered pre-execution checks (`role`, `feature-flag`, `rate-class`), ordered post-execution hooks (`audit-write`, `invariant-assert`, `notify`), and the pre-check failure mode (`deny` or `audit-only`). The schema lives at `docs/schema/piece-manifest.schema.json` and a starter instance at `app/dsl/src/main/resources/piece-manifest.yaml`. Runtime loading and enforcement are out of scope for this document and will land in T548–T552.
