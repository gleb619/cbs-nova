...
A declarative piece manifest ([`vhs-manifest.md`](vhs-manifest.md)) is the single contract that describes each guarded piece, its target (an API route, a UI button, or a DSL object), ordered pre-execution checks (`role`, `feature-flag`, `rate-class`), ordered post-execution hooks (`audit-write`, `invariant-assert`, `notify`), and the pre-check failure mode (`deny` or `audit-only`). The schema lives at [`docs/schema/piece-manifest.schema.json`](schema/piece-manifest.schema.json) and a starter instance at [`app/dsl/src/main/resources/piece-manifest.yaml`](../app/dsl/src/main/resources/piece-manifest.yaml). **Runtime loading (T548):** `PieceManifestService` loads and validates the YAML at startup via `ResourceLoader`, builds an immutable `Snapshot` indexed by piece id, target type, and API route, and serves read-only lookups (`find`, `byTarget`, `findByObject`, `findByRoute`). `reload(ServerRequest)` and `reloadManifest()` perform hot-reload with a `ReentrantLock` serialising overlapping calls — a valid candidate atomically replaces the live snapshot; on `PieceManifestValidationException` the previous snapshot stays live and per-piece errors are returned. The service is registered as a Spring bean guarded by `CbsDslManifestProperties` (`cbs.dsl.manifest.*`, default enabled, default path `classpath:piece-manifest.yaml`); when the path is blank or the resource missing, an empty snapshot is used so existing deployments are unaffected. `DslManifestHandler` exposes two REST endpoints through `DslManifestRouterConfiguration` (conditional on the `PieceManifestService` bean): `POST /api/dsl/manifest/reload` delegates to the service reload, and `GET /api/dsl/manifest/guard` returns per-button allow/deny `ManifestGuardEntry` verdicts for the current principal using `RoleResolver`. Enforcement (T549 pre-check middleware, T550 post-check hooks, T551 frontend guard, T552 object binding) is not yet landed.

- **Object-level capability enforcement (T552)** — subsumes the Epic 2 "DSL execution sandboxing" workstream: instead of a second bespoke allowlist, DSL constructs (helpers, functions, and process entry points) are guarded by `object`-target manifest pieces evaluated through the same loader and guard machinery as the API/button pieces. `ManifestObjectGuard` (gated by `cbs.dsl.manifest.object-enforcement.enabled`, default off) is consulted both in preview/explain dispatch (`ManifestObjectGuardHelperInterceptor` wrapping the fake-helper interceptor) and in production runs (the platform `ObjectGuard` seam, bridged by `ManifestObjectGuardAdapter`, consulted by `HelperManager` on every helper/function call). Preview is deny-by-default when enabled; production defaults to permissive-with-audit (`cbs.dsl.manifest.object-mode=permissive`, unlisted invocations proceed but write a `dsl_audit` row) with a `strict` knob for deny-by-default; denials surface as `CAPABILITY_DENIED` in the unified error envelope with piece/object identity, correlation id, a `dsl_audit` FAILURE row, and the `dsl.piece.object.denied` counter. This guard is layered above, and does not replace, the `OutboundUrlValidator` SSRF floor. Details: [`vhs-manifest.md`](vhs-manifest.md#object-level-enforcement-semantics-t552).
...
- **Drafts** — a draft is an uncommitted, git-tracked change to a DSL source file in the dsl-builder workspace; publish reloads and commits it (optionally pushing), discard reverts it, and history/restore read git. See [DSL Drafts](backend/drafts.md) for the status model, lifecycle, API, configuration and tests.
- **Publish approval gate (T568)** — opt-in two-person rule for draft publishing, `cbs.dsl.approval.required=true` (default `false`; when off, `POST /api/dsl/drafts/{name}/publish` behaves exactly as before). When on, `DslDraftHandler.publish()` rejects direct publish with `403 FORBIDDEN` for callers below `Role.OPERATOR` (ADMIN/OPERATOR stay exempt). An AUTHOR instead snapshots the draft as a `dsl_change_request` row (`PENDING`; prior pending request for the same definition is marked `SUPERSEDED`) via `POST /api/dsl/drafts/{name}/change-request`. A second principal at `Role.AUTHOR`+ approves with `POST /api/dsl/change-requests/{id}/approve` — the requester can never approve their own request (ADMIN exempt) — which marks the row `APPROVED` and delegates publishing to the existing `DslDraftHandler.publishPayload` flow (no duplicated logic; the publish audit entry carries `actor=approver`). `POST /api/dsl/change-requests/{id}/reject` records the reason and never publishes. Every submit/approve/reject and the gated publish attempt writes a `DslAuditService` entry. Listing via `GET /api/dsl/change-requests?definitionName=&status=`. Everything is conditional on a `DataSource` (migration `V1__init.sql` — the postgres migrations are merged into a single script; h2 keeps its per-migration files). Operator recipe: [Runbook — Publish via the approval gate](runbook.md#publish-via-the-approval-gate).
- **Notification rules engine (T565)** — `cbs.nova.starter.notification` turns published domain events into outbound notifications. `DomainEventPublisher` appends the event to `dsl_events` and then notifies every `DomainEventListener` bean with the generated row id (each call is guarded so listeners can never break publishing). `NotificationRuleEngine` loads the enabled `dsl_notification_rule` rows for the event type (priority desc), matches the in-memory filter dimensions (aggregate type, aggregate-id/definition-name globs, status), and dispatches each action to a sink on the bounded `cbsNovaNotificationExecutor` — sinks: `webhook` (reuses `WebhookDispatcher`, HMAC-signed, audited separately from the static subscription log), `slack`/`pagerduty` (single-attempt JSON POST), `email` (NOOP placeholder recording `noop`). One append-only `dsl_notification_rule_firing` row per (rule, action) records outcome/detail/duration with the originating `dsl_events.id`. Everything is conditional on a `DataSource`. HTTP surface under `/api/dsl/notifications`: rule CRUD + `POST /rules/{id}/enabled` (secrets are write-only), `GET /channels`, `GET /fire-log`, and `POST /test` (synchronous synthetic run-lifecycle event through the same matching+sinks path, persisted nowhere). Rule mutations audit via `DslAuditService` (`notification_rule.create/update/delete/toggle`).
...
Run attribution and correlation are stored on the `dsl_runs` table: migration `V1__init.sql` (the postgres migrations are merged into this single script) defines the `triggered_by` and `correlation_id` columns. See [Runtime Engine — Run idempotency](dsl/runtime.md#run-idempotency) and [Correlation id](dsl/runtime.md#correlation-id) for the header semantics.
...
- ~~**No update / modify endpoint.** The cron and timezone are immutable after creation; changing the schedule requires delete + recreate. The service does not call `updateSchedule(...)`.~~ **Update is now supported.** `PATCH /api/dsl/schedules/{definition}` (`DslScheduleHandler.update`) updates a schedule's cron/timezone in place via `ScheduleHandle.update(ScheduleUpdate)`, reusing `create`'s cron/timezone validation and writing an audit row on success. Missing schedule → `404`.
...
Definition-version attribution (T492): migration `V1__init.sql` adds a nullable `definition_hash` column, stamped at run submission (`RunDefinitionHash`, called from `TemporalDslProcessService.startProcess`) and exposed as `ExecutionDto.definitionHash`. **This is DESCRIPTOR identity, not full logic identity** — it is the same sha256 over the Jackson-serialized `DslDescriptor` (taskQueue / version / timeouts) that the preview cache keys on, so two functionally different definitions with the same descriptor collide. It is null for historical rows and for runs whose descriptor cannot be resolved (never a run failure). A true content hash computed at publish/reload time is a planned Epic 5 follow-up.
...
Operator recipe: [Runbook — Signal a running process](runbook.md#signal-a-running-process).
...
- [DSL Drafts](backend/drafts.md) — what a draft is (git-changed DSL file in the builder worktree), status model, lifecycle, API, configuration and tests.
