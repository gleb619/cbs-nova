...
- **Drafts** — a draft is an uncommitted, git-tracked change to a DSL source file in the dsl-builder workspace; see [DSL Drafts](backend/drafts.md) for the status model, draft/file/vcs routes, and the as-is (`.workbench/` JSON markers) vs target (git-backed) gap.
...
- [DSL Drafts](backend/drafts.md) — what a draft is (git-changed DSL file in the builder worktree), status model, API, gap analysis, and test matrix.
