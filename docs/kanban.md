...
| T221 | Done | Remove `synchronized` fallback from `SingletonSupport.SingletonScope` | backend — `onLameWay` recovery path (triggered on `ConcurrentHashMap` recursive-update detection) still uses `synchronized (instances)` double-checked locking even though `instances` is already a `ConcurrentHashMap`; replace with atomic `putIfAbsent`, consistent with the T215 `GlobalManager` cleanup. Foundational (`DslConfig`/`Replaceable` machinery) — keep diff scoped to `onLameWay`, run full `:dsl:test`. See plan. | High | loop | - | - | `./docs/plans/T221-singleton-support-lock-free.md` |
