...
| T224 | In Progress | Add dedicated tests for `CompilingDslDefinitionLoader`/`ServiceLoaderDslDefinitionLoader` | backend — T207 split `DefinitionLoader` into an interface with two implementations; only the composite `DefinitionLoaderTest` exists today, the two implementations are only covered indirectly. Test-only change. See plan. | Medium | loop | - | - | `./docs/plans/T224-definition-loader-impl-tests.md` |
