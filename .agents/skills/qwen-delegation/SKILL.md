...
description: >
  Delegate implementation tasks to Qwen via MCP tool. The agent acts as
  orchestrator: analyzes requirements, writes a task spec, invokes the qwen_run
  MCP tool, then verifies the result. Use for any backend/frontend/build/infra
  implementation task.
---

# Skill: Qwen Delegation via MCP

## Overview

This skill delegates implementation tasks to Qwen through the `qwen_run` MCP tool.

**MCP Tool:** `qwen_run`  
**Parameters:**
- `task_name` (required): Task identifier (kebab-case, without .md extension)
- `timeout_sec` (optional): Timeout in seconds (default: 300)
- `args` (optional): Additional CLI arguments (e.g., `['-y', '--output-format', 'text']`)
...
│
▼
[3. Agent: Call MCP tool qwen_run]
│   Tool: qwen_run
│   Args: { task_name: "{task-name}", timeout_sec: 300 }
│
▼
[4. Qwen: Reads task, implements, writes result file]
...
└── FAIL ──► [6. Agent: Add Retry Notes to task file]
                 │
                 ▼
           [7. Re-invoke qwen_run with same task_name]
                 │
                 ├── PASS ──► Done ✅
...
After writing the task file, invoke the `qwen_run` MCP tool:
...
2. Execute Qwen with the task instructions
...
- Re-invoke `qwen_run` with the same `task_name` — Qwen will read the updated task file
