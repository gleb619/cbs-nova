...
description: >
  ...
  then waits. The user executes the task manually and reports back. The agent
  verifies the report and writes the result file. Use for any
  backend/frontend/build/infra implementation task where the user is the
  executor.
---

# Skill: Agent-Planned / Human-Executed Task Delegation

## Roles

| Actor | Role         | Responsibility                                              |
|-------|--------------|-------------------------------------------------------------|
| Agent | Orchestrator | Analyze, plan, write task spec, verify report, write result |
...
│
▼
[2. Agent: Write docs/tasks/{task-name}.md — print it for user]
│
▼
[3. User: Implement task, run all verification commands, paste output]
│
▼
[4. Agent: Verify output — all commands exit 0, files exist, conventions met]
│
├── PASS ──► [5a. Agent: Write docs/results/{task-name}.result.md] ──► Done ✅
└── FAIL ──► [5b. Agent: Append Retry Notes to task file, ask user to re-run]
