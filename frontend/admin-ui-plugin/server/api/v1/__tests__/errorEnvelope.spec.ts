...
const errorRoutes: ErrorRouteCase[] = [
  {
    ...
    handler: async () => (await import('../dsl/run/[name].post')).default(fakeEvent),
    params: { name: 'runDef' },
    body: { input: {} },
  },
  {
    name: 'POST /api/v1/dsl/preview/:name',
    handler: async () => (await import('../dsl/preview/[name].post')).default(fakeEvent),
    params: { name: 'previewDef' },
    body: { input: {} },
  },
  {
    name: 'POST /api/v1/dsl/explain/:name',
    handler: async () => (await import('../dsl/explain/[name].post')).default(fakeEvent),
    params: { name: 'explainDef' },
    body: { input: {} },
  },
  {
    name: 'GET /api/v1/executions/:id',
    handler: async () => (await import('../executions/[id].get')).default(fakeEvent),
    params: { id: 'exec-123' },
  },
  {
    name: 'POST /api/v1/executions/:id/cancel',
    handler: async () => (await import('../executions/[id]/cancel.post')).default(fakeEvent),
    params: { id: 'exec-123' },
  },
  {
    name: 'GET /api/v1/dsl/drafts/:name',
    handler: async () => (await import('../dsl/drafts/[name]/index.get')).default(fakeEvent),
    params: { name: 'draftDef' },
  },
  {
    name: 'POST /api/v1/dsl/drafts/:name/save',
    handler: async () => (await import('../dsl/drafts/[name]/save.post')).default(fakeEvent),
    params: { name: 'draftDef' },
    body: { source: 'foo: bar' },
  },
  {
