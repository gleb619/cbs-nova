    ...
    <div v-for="(err, idx) in props.errors" :key="idx"
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div v-if="err.code" class="text-xs text-gray-500 mt-1">Code: <span class="font-mono">{{ err.code }}</span></div>
          <div v-if="err.stackTrace" class="mt-2">
            ...
            <pre v-if="expanded.has(idx)"
                 class="mt-2 text-[11px] bg-gray-50 border border-gray-200 rounded p-2 overflow-auto max-h-64 whitespace-pre"><code>{{ err.stackTrace }}</code></pre>
          </div>
        </div>
      </div>
    </div>
