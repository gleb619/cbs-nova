...
class SumValuesHelperTest {
  ...
  @Test
  void sumsValues() {
    var ctx = SimpleContext.of(new SumValuesIn(List.of(1.0, 2.0, 3.0)), ExecutionMode.PREVIEW);
  ...
  void returnsZeroForEmpty() {
    var ctx = SimpleContext.of(new SumValuesIn(List.of()), ExecutionMode.PREVIEW);
    assertThat(helper.execute(ctx).value().sum()).isEqualTo(0.0);
  }

  @Test
  void returnsZeroForNullList() {
    var ctx = SimpleContext.of(new SumValuesIn(null), ExecutionMode.PREVIEW);
    assertThat(helper.execute(ctx).value().sum()).isEqualTo(0.0);
  }
}
