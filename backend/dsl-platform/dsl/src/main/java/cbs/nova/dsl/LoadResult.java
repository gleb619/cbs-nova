 ...
 * <p>
 * Replaces the bare {@code int} count the loader used to return so callers (reload admin endpoint,
 * workbench publish, logs) can surface <em>what</em> was loaded — the counts and names per
 * {@link DslType}, not just a total.
 *
 * @param processes
 *          the {@code PROCESS} definition names that were registered
 * @param transactions
 *          the {@code TRANSACTION} definition names that were registered
 * @param functions
 *          the {@code FUNCTION} definition names that were registered
 */
