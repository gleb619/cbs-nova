 ...
 *
 * <p>
 * The builder emits two wire shapes depending on the endpoint:
 *
 * <ul>
 ...
 *       CompileErrorResponse}), where {@code error} carries the machine code and {@code message} is
 * folded into {@code diagnostics[0]}.
 * </ul>
 *
 * <p>
 * All fields are nullable so a single record can deserialize either shape. Callers should use
 * {@link #primaryCode()} and {@link #primaryMessage()} to pick the right value regardless of shape.
 *
 * @param code
 *          machine-readable error code (general shape)
 * @param message
 *          human-readable message (general shape)
 * @param error
 *          machine-readable error code (compile shape)
 * @param diagnostics
 *          list of diagnostic strings; for compile failures the first entry is the human message
 */
