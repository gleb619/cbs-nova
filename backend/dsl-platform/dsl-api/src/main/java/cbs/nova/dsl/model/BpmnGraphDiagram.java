...
final class BpmnGraphDiagram {

  private BpmnGraphDiagram() {
  }
  ...
  static @NonNull String render(@NonNull DslType kind, @NonNull String name,
    ...
    return switch (kind) {
      case PROCESS, TRANSACTION -> generateXml(name, hasCompensation, externalCalls, callCounts);
      default -> generateXml(name, false, externalCalls, callCounts);
    };
  }
  ...
  private static @NonNull String generateXml(String name, boolean hasCompensation,
          @Nullable List<Map<String, Object>> externalCalls,
          @Nullable Map<String, Integer> callCounts) {
    var template = """
