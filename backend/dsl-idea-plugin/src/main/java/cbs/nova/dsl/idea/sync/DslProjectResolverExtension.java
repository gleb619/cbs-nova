 ...
 * platform version, so a static field is a safe, simple hand-off. It is keyed per
 * {@link ExternalSystemTaskId} -- the id of the RESOLVE_PROJECT external-system task driving the
 * current Gradle sync, obtained from {@code resolverCtx.getExternalSystemTaskId()} -- rather than
 * being a single shared bucket, so that concurrent syncs of different projects (or, in principle,
 * concurrent module resolution within the same sync) never clobber or cross-deliver each other's
 * discovered directories. {@link ConcurrentHashMap} plus {@link ConcurrentHashMap#newKeySet()}
 * value sets keep both the per-task accumulation and the final drain thread-safe; the drain uses
