...
class MassOpBuilder(
...
) : MassOperationDefinition {
    private var _category: String = ""
    override val category: String get() = _category

    private val _triggers = mutableListOf<TriggerDefinition>()
    override val triggers: List<TriggerDefinition> get() = _triggers.toList()
    ...
    override val lock: LockDefinition? get() = _lock

    private var _contextBlock: (MassOperationContext) -> Unit = {}
    override val contextBlock: (MassOperationContext) -> Unit get() = _contextBlock
    ...
    override val onPartial: ((Signal) -> Unit)? get() = _onPartial

    private var _onCompleted: ((Signal) -> Unit)? = null
    override val onCompleted: ((Signal) -> Unit)? get() = _onCompleted
...
fun massOperation(
    code: String,
    block: MassOpBuilder.() -> Unit,
): MassOperationDefinition = MassOpBuilder(code).apply(block)
