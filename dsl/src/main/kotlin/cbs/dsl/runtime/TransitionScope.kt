...
class TransitionScope {
    ...
    infix fun Pair<String, String>.on(action: Action): TransitionActionBuilder = TransitionActionBuilder(first, second, action)

    /** TransitionActionBuilder { event block } → TransitionBuilder (registers rule) */
    operator fun TransitionActionBuilder.invoke(eventBlock: EventBuilder.() -> Unit): TransitionBuilder {
    ...
    infix fun TransitionBuilder.onFault(block: suspend TransactionsScope.() -> Unit): TransitionBuilder {
    ...
    private fun TransitionBuilder.build(): TransitionRule = TransitionRule(from, to, on, event, onFaultState, onFaultBlock)
}
