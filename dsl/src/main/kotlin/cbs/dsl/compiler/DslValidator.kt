...
class DslValidator {
    ...
    if (fileName.endsWith(".transaction.kts")) {
      if (registry.transactions.isEmpty()) {
        errors +=
            ValidationError(
                fileName,
                "Transaction script must define exactly one 'transaction { }' block",
            )
      } else if (registry.transactions.size > 1) {
        errors +=
            ValidationError(
                fileName,
                "Transaction script defines multiple 'transaction { }' blocks",
            )
      }
    ...
    if (scriptContent.isNotEmpty()) {
      val allCodes =
          registry.workflows.keys +
              ...
              registry.transactions.keys +
              registry.massOperations.keys +
  ...
  private fun validateWorkflow(
    ...
    if (wf.initial.isBlank() || wf.initial !in stateSet) {
      errors +=
          ValidationError(fileName, "Workflow '$code': initial state '${wf.initial}' not in states")
    }
    ...
    for (terminal in wf.terminalStates) {
      if (terminal !in stateSet) {
        errors +=
            ValidationError(fileName, "Workflow '$code': terminal state '$terminal' not in states")
      }
    }
