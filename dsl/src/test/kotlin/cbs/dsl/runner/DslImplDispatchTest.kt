...
class DslImplDispatchTest {
        ...
        ImplRegistry().apply {
          ...
          register(
              TestHelper(
                  ...
                  executeBlock = { params ->
                    mapOf("loanId" to params["loanId"], "currency" to "USD")
                  },
              )
          )
        }
  ...
  fun `shouldCallTestHelperForLoanConditionsWhenResolvedByName`() {
    ...
    assertTrue(
        loanConditions is Map<*, *>,
        "Expected loanConditions to be a Map, but was: ${loanConditions?.javaClass}",
    )
  }
  ...
  private fun readFile(path: String): String =
      javaClass.classLoader.getResource(path)?.readText() ?: error("Resource not found: $path")
}
