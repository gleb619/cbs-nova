    ...
    fun setUp() {
        ...
        val eventDef =
            EventBuilder("LOAN_DISBURSEMENT").apply {
                ...
                context { ctx ->
                    ctx["customer"] = ctx.helper("FIND_CUSTOMER", mapOf("id" to ctx.eventParameters["customerId"]!!))
                }
                transactions {
                    val k = step(kycTx)
                    val d = step(debitTx)
    ...
    fun `shouldResolveHelperChainWhenHelperCallsAnotherHelper`() {
        val enrichHelper =
            object : HelperDefinition {
                ...
                override fun execute(input: HelperInput): HelperOutput {
                    ...
                    val base =
                        (findDef.execute(MapHelperInput(mapInput.params, mapInput.baseContext)) as AnyHelperOutput).value
                            as Map<String, Any>
        ...
        val chainEvent =
            EventBuilder("CHAIN_EVENT").apply {
                context { ctx ->
                    ctx["result"] = ctx.helper("ENRICH_CUSTOMER", mapOf("id" to "C1"))
                }
            }
        ...
        @Suppress("UNCHECKED_CAST")
        val result = record.enrichment["result"] as Map<String, Any>
